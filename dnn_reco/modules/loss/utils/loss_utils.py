...
def add_logging_info(data_handler, shared_objects):
    ...
    if 'rmse_values_trafo' not in shared_objects:
        ...
        mse_values_trafo = tf.reduce_mean(input_tensor=tf.square(y_diff_trafo), axis=0)
        ...
        tf.compat.v1.summary.scalar('Benchmark', tf.reduce_sum(input_tensor=rmse_values_trafo, axis=0))
