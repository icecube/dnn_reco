...
dir_original = 'test_data/dnn_reco_test_01_base_v0.2.0'
        ...
        for key in keys:
            ...
            for k in df_original.columns:
                if 'runtime' not in k:
                    ...
                    runtime_orig = np.mean(df_original[k].values) * 1000.
                    runtime_orig_std = np.std(df_original[k].values) * 1000.
                    runtime_test = np.mean(df_test[k].values) * 1000.
                    runtime_test_std = np.std(df_test[k].values) * 1000.
                    ...
                    if np.abs(runtime_orig - runtime_test) > max_dev:
                        ...
                        print(msg.format(
                            runtime_orig, runtime_orig_std,
                            runtime_test, runtime_test_std))

print('\n====================')
print('=== Summary ========')
print('====================')
if got_warning:
    print('=== ' + bcolors.WARNING + 'Warnings: {}'.format(got_warning)
          + bcolors.ENDC)
else:
    print('===' + bcolors.OKGREEN + ' Warnings: {}'.format(got_warning)
          + bcolors.ENDC)
if passed_test:
    print('===' + bcolors.OKGREEN + ' Passed:   {}'.format(passed_test)
          + bcolors.ENDC)
else:
    print('===' + bcolors.FAIL + ' Passed:   {}'.format(passed_test)
          + bcolors.ENDC)
print('====================\n')
